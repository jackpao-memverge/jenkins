...
'{"text": \
"NIGHTLY REGRESSION snapshot suite: \n \
BUILD_LOCATION: %s \n \
JOBS LIST PASSED: \n%s \n \
...
node ("cicd_vm") {
   ...
   try{
       MVTEST_GROUP = "[Nightly] Snapshot test suite, basic"
   ...
   try{
       MVTEST_GROUP = "[Nightly] Snapshot test suite, Hugepage cache 5G"
   ...
   try{
    ...
    try{
       MVTEST_GROUP = "[Nightly] Snapshot test mm bin installation suite"
...
def snapshot_suite_all(list) {
    for (int i = 0; i < list.size(); i++) {
           ...
           b1 = build job: "pipeline_mvmalloc_nightly_test", propagate: false, parameters: [
