...
node ("cicd_vm") {
   ...
   try{
       ...
       stage ("Compile mvcriu and mvmalloc ${BUILD_LABEL} branch")
       {
          ...
          build job: "pipeline_nightly_build_stage", parameters: [string(name: "build_branch", value: "${BUILD_LABEL}"), 
          [$class: "LabelParameterValue", name: "node", label: "${RHEL_VER}"], string(name: "RHEL_VER", value: "${RHEL_VER}"), 
          string(name: "Pipeline_name", value: "Pipeline_master"), string(name: 'criu_branch', value: "${criu_branch}")]
          
          echo "rhel8 build"
          build job: "pipeline_nightly_build_stage", parameters: [string(name: "build_branch", value: "${BUILD_LABEL}"), 
          [$class: "LabelParameterValue", name: "node", label: "rhel8"], string(name: "RHEL_VER", value: "rhel8"), 
          string(name: "Pipeline_name", value: "Pipeline_master"), string(name: 'criu_branch', value: "${criu_branch}")]
       ...
       stage ("Install basic workflow test")
       {
          ...
          string(name: 'TEST_SUITE', value: 'run_all'), string(name: 'HOSTS_DAX_MAP', value: '10.0.1.165=/dev/dax0.0,/dev/dax1.0'), 
       ...
       stage ("Functional redis test")
       {
        ...
        //   string(name: "BUILD_LOCATION", value: "/nfs/memverge/home/mvm/${BUILD_LABEL}_tmp/${RHEL_VER}/*/mvmm*x86_64.tgz"), 
           string(name: "BUILD_LOCATION", value: "/nfs/memverge/home/mvm/${BUILD_LABEL}_tmp/rhel8/*/mvmm*x86_64.tgz"), 
       ...
       stage ("Functional kdb test, run as non sudo user")
       {
       ...
       stage ("MVMCLI basic workflow")
       {
           ...
           value: "/memverge/home/mvm/${BUILD_LABEL}_tmp/${RHEL_VER}"), string(name: "BUILD_DATE", value: "*"), string(name: "TEST_SUITE", value: "mvmcliBasicWorkFlows"), 
           string(name: "KX_HOME", value: "/memverge/automation/q/l64"),string(name: 'HOSTS_DAX_MAP', value: '10.0.1.94=/dev/dax0.0,/dev/dax1.0'),
           string(name: "MVTEST_BRANCH", value: "${MVTEST_BRANCH}"), string(name: "BUILD_PACKAGE_NAME", value: "mvmm*x86_64.tgz"),
           booleanParam(name: 'TCMS_DRY_RUN', value: false), booleanParam(name: 'TCMS_TRACE', value: false)]
       }
       stage ("Package")
       {
          ...
          build job: 'pipeline_pkg_rhel78_same_dir', parameters: [string(name: 'BUILD_BRANCH', value: "${BUILD_LABEL}")]
            ...
            subject: "Pipeline Nightly Build", to: 'jack.pao@memverge.com'
            if(currentBuild.result == 'FAILURE')
                sh ''' 
                curl -X POST -H 'Content-type: application/json' --data '{"text":"Nightly build status--master *FAILURE* https://104.184.156.164:8888/view/Pipeline_view/job/Pipeline_master\nChangelog: http://niles.eng.memverge.com/automation/changelog/mvmalloc_changelog http://niles.eng.memverge.com/automation/changelog/mvcriu_changelog"}' \
                ####slack hook url
                
                '''
            else
                sh ''' 
                curl -X POST -H 'Content-type: application/json' --data '{"text":"Nightly build status--master *SUCCESS* https://104.184.156.164:8888/view/Pipeline_view/job/Pipeline_master\nChangelog: http://niles.eng.memverge.com/automation/mvm/master/latest/mvmalloc_changelog http://niles.eng.memverge.com/automation/mvm/master/latest/mvcriu_changelog"}' \
                ####slack hook url
